...
export async function POST(req: NextRequest) {
  ...
  const contest = await prisma.contest.create({
    data: {
      ...
      teamContest: teamContest === true,
      teamSize: teamContest === true ? (Number(teamSize) || 2) : 1,
      organizerId: session.user.id,
      status: isAdmin ? 'APPROVED' : 'PENDING',
      problems: {
        create: problems.map((p: { title: string; content: string; answer: string; extraAnswers?: string[]; subAnswers?: { label: string; answer: string; extra?: string[] }[]; points: number; imageUrls?: string[]; allowRetry?: boolean }, i: number) => ({
          label: labels[i] ?? String(i + 1),
          title: p.title,
          content: p.content,
          answer: p.answer,
          extraAnswers: JSON.stringify(p.extraAnswers ?? []),
          subAnswers: JSON.stringify(p.subAnswers ?? []),
          points: p.points ?? 100,
