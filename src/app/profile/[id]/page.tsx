...
export default function PublicProfilePage() {
  ...
  if (!profile) return (
    <div className="max-w-3xl mx-auto px-4 py-16 text-center text-text-secondary">유저를 찾을 수 없습니다</div>
  )
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-5">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              ...
              {isMe && (
                <Link href="/profile" className="px-3 py-1.5 rounded-lg border border-border text-sm text-text-secondary hover:text-text-primary hover:border-border-2 transition-colors">
                  프로필 편집
                </Link>
              )}
            </div>
      ...
      <div className="bg-surface border border-border rounded-2xl p-5">
        <h2 className="text-sm font-semibold text-text-secondary mb-3">활동 스트릭</h2>
      ...
      {subjectEntries.length > 0 && (
        <div className="bg-surface border border-border rounded-2xl p-5">
          <h2 className="text-sm font-semibold text-text-secondary mb-3">과목별 기여</h2>
          ...
          {profile.posts.length === 0 ? (
            ...
            <div className="space-y-1.5">
              {profile.posts.slice(0, 10).map((p) => (
                <Link key={p.id} href={`/post/${p.id}`} className="flex items-center justify-between p-2 rounded-lg hover:bg-surface-2 transition-colors group">
                  <div className="min-w-0">
                    <span className="text-xs px-1.5 py-0.5 bg-accent/10 text-accent rounded mr-1.5">
                      {SUBJECTS[p.subject as keyof typeof SUBJECTS]?.short ?? p.subject}
                    </span>
                    <span className="text-xs text-muted">{timeAgo(p.createdAt)}</span>
                  </div>
                  {p.pointsAwarded > 0 && (
                    <span className="text-xs text-emerald-500 shrink-0">+{p.pointsAwarded}pt</span>
                  )}
                </Link>
              ))}
            </div>
